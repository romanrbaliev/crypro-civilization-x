...
const processHireReferralHelper = (state: GameState, payload: { referralId: string; buildingId: string }): GameState => {
  ...
  // Также проверяем, есть ли уже принятый помощник для этого реферала и здания
  const existingActiveHelper = state.referralHelpers.find(
    h => h.helperId === referralId && h.status === 'accepted' && h.buildingId === buildingId
  );
  
  if (existingActiveHelper) {
    console.log('Помощник уже работает на этом здании', existingActiveHelper);
    safeDispatchGameEvent("Этот помощник уже работает на выбранном здании", "warning");
    return state;
  }
  
  // Проверяем, есть ли уже принятый помощник для этого реферала на другом здании
  const existingActiveHelperOnOtherBuilding = state.referralHelpers.find(
    h => h.helperId === referralId && h.status === 'accepted'
  );
  
  if (existingActiveHelperOnOtherBuilding) {
    // Если помощник уже работает на каком-то здании, сначала увольняем его
    const updatedHelpers = state.referralHelpers.filter(h => h.id !== existingActiveHelperOnOtherBuilding.id);
  ...
  console.log('Создан новый помощник:', newHelper);
  console.log('Здание для помощника:', buildingId);
